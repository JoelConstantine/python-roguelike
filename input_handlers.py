...
class HistoryViewer(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[MainGameEventHandler]:
        ...
        if event.sym in CURSER_Y_KEYS:
            adjust = CURSER_Y_KEYS[event.sym]
        ...
        else: # Any other key moves back to game state
            return MainGameEventHandler(self.engine)
        return None
...
class LevelUpEventHandler(AskUserEventHandler):
    ...
    def ev_mousebuttondown(
        ...
        """Don't allow the player to click on the menu, like normal"""
        return None
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        Will move to a different position based on where the player is locatied,
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if 0 <= index <= 26:
            try:
                ...
                self.engine.message_log.add_message("Invalid entiry.", color.invalid)
                return None
